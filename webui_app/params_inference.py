...
def _polish(reg, P):
    """输出后处理（提亮 / 空气感）。"""
    reg(P(
        key="polish_enable", group="polish", label="启用输出后处理",
        kind="checkbox", default=True,
        summary="对**合成输出**做听感提亮。关掉则与模型原始结果完全一致（最保真）。",
        info="合成页「输出后处理」区。处理结果另存为 <code>*_polish.wav</code>，"
             "原始输出保留，方便 A/B 对比。",
        affects="听感亮度、清晰度；不改变模型本身",
        detail_md="""
**先澄清一个常见误解**：IndexTTS2 的输出固定 **22050 Hz**，Nyquist 上限 11 kHz，
**11 kHz 以上不可能有能量** —— 这是模型设计。实测证据：

- 增强链是**频谱透明**的（处理前后各频段能量完全一致，同样的 96.94 / 2.29 / 0.78）；
- CFM LoRA 反而让高频**略多**（8~11 kHz 能量从 5.27% 升到 5.91%，
  谱质心 3304 → 3562 Hz）；
- 官方示例音频本身在 8 kHz 以上几乎没有能量（0.05%）。

所以拿 48 kHz 的原始素材去比，听感上会觉得「发闷、不通透」是正常的，
**不是处理链把高频裁掉了**。这里做的是听感补偿：

| 处理 | 作用 |
|---|---|
| 高通 60 Hz | 去掉低频隆隆声，不占动态余量 |
| presence 提升 | 4.5 kHz 以上平滑抬升，人耳对这段最敏感（齿音、咬字清晰度） |
| 谐波激励 | 从 5 kHz 以上生成谐波、只叠加 9 kHz 以上的部分，缓解「高频被切断」的封闭感 |
| 峰值对齐 | 避免削波，也避免下游播放器自行衰减 |

**它不伪造外来细节**：presence 是 EQ，激励是把**这段音频自身**的高频做非线性延展。
想要最大保真就把这一项关掉。
""",
        pitfall_md="""
presence 超过 5 dB 齿音会发刺；激励超过 0.15 会有明显失真感。
建议先只开 presence 2~4 dB 听着定，再决定要不要加空气感。
""",
    ))

    reg(P(
        key="polish_presence_db", group="polish", label="提亮 presence",
        kind="slider", default=2.5, minimum=0.0, maximum=6.0, step=0.5,
        unit="dB",
        summary="4.5 kHz 以上平滑抬升多少。这个区间决定「清楚」和「亮」。",
        info="2~4 dB 明显更亮更清楚；超过 5 齿音容易发刺。0 = 不动。",
        affects="听感亮度、清晰度（谱质心可量化）",
        detail_md="""
用升余弦做**一个倍频程的平滑过渡**（4.5 kHz 处起、2.25 kHz 处完全过渡完），
不会在转折频率上产生可闻的「硬边」。

实测（对一段 22050 Hz 的合成音频）：

| presence | 4-8 kHz 能量 | 8-11 kHz 能量 | 谱质心 |
|---|---|---|---|
| 0 dB（中性） | 2.71% | 0.32% | 3376 Hz |
| +3 dB | 5.24% | 0.61% | **3917 Hz** |
| +3 dB + 激励 0.12 | 5.24% | 0.99% | **4207 Hz** |

合成页会显示处理前后的谱质心 —— **「亮了没有」有客观数字**，不靠感觉争。
""",
    ))

    reg(P(
        key="polish_exciter", group="polish", label="空气感激励",
        kind="slider", default=0.08, minimum=0.0, maximum=0.3, step=0.01,
        summary="从高频成分生成谐波、只叠加 9 kHz 以上的部分。",
        info="0.05~0.15 是安全区；再高会明显失真。0 = 关闭。",
        affects="11 kHz 附近的「空气感」与通透度",
        detail_md="""
做法：把 5 kHz 以上的成分取出来 → 过 `tanh` 非线性生成谐波 → 只保留
9 kHz 以上 → 以很低的电平叠加回原信号。

为什么只叠 9 kHz 以上：**9 kHz 以下本来就有真实内容**，在那里加谐波只会让声音变糊；
而 9~11 kHz 正是模型输出最「空」的一段，补上它才能缓解「高频被切断」的封闭感。

> 实现上刻意**没有**用 `preemphasis`/`deemphasis` 那一对：`deemphasis` 是
> `1/(1-0.95z⁻¹)`，直流增益高达 20 倍，把它作用在未预加重的主信号上会把低频
> 整体抬起来 —— 实测谱质心从 3355 Hz 掉到 788 Hz、99.98% 能量挤进 0~4 kHz，
> 声音直接闷掉。两者必须成对作用在同一路信号上，所以这里改用真正的滤波器。
""",
        pitfall_md="""
激励是**非线性**处理，电平偏高时会引入互调失真（听感「毛刺」「沙」）。
调大之前先把 presence 调够 —— 多数情况下「不够亮」是 presence 不足，而不是缺空气感。
""",
    ))

    reg(P(
        key="denoise_keep_high_hz", group="polish",
        label="降噪时保留高频的下限",
        kind="number", default=8000.0, minimum=0.0, unit="Hz",
        summary="这条线以上的频率在降噪时保留原始信号，避免把细节一起抹掉。",
        info="0 = 关闭保护（整带都降噪，容易发闷）。默认 8000 Hz。",
        affects="降噪后的「通透度」",
        detail_md="""
noisereduce 是**宽带**门限：它在所有频段一起估计噪声底，而人声的齿音、气声、
咬字细节恰好集中在 7~11 kHz —— 一起压下去，听感就是「闷」「哑」。

所以增强链现在做**分频段降噪**：把降噪结果与原始信号按频率拼回来，
这条线以下用降噪结果（去掉嘶声、嗡声、房间底噪），以上用原始（细节一点不动）。

实测参考素材里 8~11 kHz 只占约 5% 的能量，但人耳对它的有无非常敏感 ——
它决定「通透」还是「发闷」。
""",
        pitfall_md="""
如果素材的高频本身就是噪声（例如磁带嘶声、8 kHz 以上的电子干扰），
把这条线调低一些让降噪覆盖到它。反过来，干净的录音保持 8000 即可。
""",
    ))

    reg(P(
        key="enhance_highpass_hz", group="polish", label="增强时的高通",
        kind="number", default=60.0, minimum=0.0, unit="Hz",
        summary="增强/参考音频处理时，先去掉这条线以下的低频。",
        info="默认 60 Hz。0 = 关闭。语音的有效低频下限约 80 Hz。",
        affects="清晰度与响度归一的准确度",
        detail_md="""
60 Hz 以下基本是人声之外的东西：桌面震动、空调风噪、直流偏移、
房间的隆隆声。它们不参与听感，却会：

- 占掉动态余量（峰值归一后被整体压电平）；
- 把 RMS 响度算高（归一化因此把语音本体的电平压低）。

一个二阶高通就能解决，代价是 60 Hz 以下的信息 —— 语音里没有。
""",
    ))
