...
- **全部功能有可复现的回归测试**：**18 套**回归与静态校验、**1428 项断言全部通过**，另有三份真机端到端验收（训练体系 33 项 · 一键三连 70 项 · 参数记忆 26 项），均见 [§9 测试与验证](#9-测试与验证)。
...
- **导出片段之后再点一次会失灵？** 已经修了。导出/「设为主素材」会把主素材换成刚
  切出来的短片段，而候选片段里存的是**相对原始长音频**的秒数偏移 —— 再切一次就
  会按「43~55 秒」去切一个 12 秒的文件，越界切出 0 个采样点，写成一个**只有
  文件头的 44 字节 wav**（能打开、听不到声音），后续体检判「音频为空」，
  增强与入库跟着全失败。
  现在：切分范围超出音频长度会**直接报错**并告诉你「这些秒数是按另一个音频算的，
  请重新扫描」；换主素材时候选片段自动作废；`save_audio` 拒绝写出 0 采样点；
  体检不过的音频不许进音色库。**「设为主素材」还会顺手体检该片段**，
  报告直接显示在下方，可以立刻接着做增强或入库。
