...
1. **一个真正好用的可视化控制台。** 上游的 `webui.py` 是单页长表单，参数挤在一起、没有提示、没有工作流。本项目将其重构为 **13 个职责清晰的功能页**：每个参数都带悬停提示与参数手册，并新增参考音频工作台、批量生产、预设管理、模型下载、系统监控、产物清理等页面。
...
| 磁盘产物盘点与安全清理 | ❌ 手动翻目录 | ✅ 清理页：逐项明细 + 确认弹窗 + 越权防护 |
...
- **参数记忆** —— 合成页参数改动自动落盘，重启后自动恢复（含上传音频副本）；亦可存成命名配置档与预设互通。
...
- **产物清理页**：一键三连的评选产物、A/B 评测、训练运行（adapter + 保险库 checkpoint）、LoRA 导出、数据集、音色库条目、工作台切片、合成音频、批量任务、缓存 —— 全部逐条盘点体积与说明，可单项删除或整类清空，删除前强制弹窗列出明细；底模、项目代码、预设、日志永不出现在清理范围里，训练中的 run 自动锁定。
...
- **全部功能有可复现的回归测试**：**18 套**回归与静态校验、**1428 项断言全部通过**，另有四份真机端到端验收（训练体系 33 项 · 一键三连 70 项 · 参数记忆 26 项 · 工作台切片交互 18 项），均见 [§9 测试与验证](#9-测试与验证)。
...
<td><img src="assets/ui/13_cleanup.png"/><br/><b>🧹 清理</b> —— 全部产物逐条盘点（体积 / 路径 / 来源），单项删除或整类清空，删除前弹窗确认明细</td>
</tr>
<tr>
...
# 4) 启动
uv run webui_pro.py          # 本项目：13 页控制台
...
4. **那条超长原件不会被送去识别。** 切片后数据集里仍然留着原件（溯源用），
   但它超过训练上限、永远不可能参与训练。此前识别阶段会把「没有文本的样本」
   全部排队转写 —— 包括这条 1739 秒（29 分钟）的原件，而且它排在**第一位**，
   whisper 第一条就要转 29 分钟，看起来就是「卡死」。
   现在识别**只转写能成为训练数据的样本**（时长在 1~20 秒内），
   原件会被跳过、标注「不参与训练」，并在报告里说明跳过了哪几条。

   > 注意这里**不能按状态过滤**：体检逻辑里「无文本」优先于「过长」，
   > 那条原件的状态是 `no_text` 而不是 `too_long`，按状态筛根本拦不住。

5. **退化文本会被丢弃。** whisper 在笑声 / 音乐 / 噪声片段上会「幻听」——
   实测一条 **11.4 秒**的音频被转成了 **446 个「哈」**。这种文本必须挡在训练之外：
   它既等于教模型「输出一长串同一个字」，又会在评测里把合成拖成几分钟
   （446 字触发低显存自动分块 = 12 块，每块都可能陷入重复循环，
   一条样本 8~11 分钟 → 6 个候选近一小时）。

   识别阶段与筛选阶段**各判一次**，任一命中即整条剔除，并在报告里说明原因：

   | 判据 | 例子 |
   |---|---|
   | 整条只有同一个字 | 446 个「哈」 |
   | 单字占比 ≥ 50% | 190 个「呵」/218 字 |
   | 6 个以上连续重复字符 | 「读书时的」重复 30 遍 |
   | 长文本不同字符占比 < 15% | 149 字里只有 5 个不同字符 |
   | 命中 whisper 幻听套话 | 「请不吝点赞…」「Thanks for watching」 |
   | 有效字符不足 | 纯标点、空文本 |

   > 判据只针对**明显退化**的形态：`哈哈,工作狂,我就知道你会这么说。` 这类
   > 正常句子（含语气词）不会被误伤 —— 探针里专门钉了这条。
...
合成长度由文本决定，一条超长文本能让单个候选的评测从十几秒变成十几分钟。
所以一键三连的择优用 `pick="short"`：优先取**文本最短**的样本
（同长度时取音频短的），所有候选评的是同一批样本，比较依然公平。
「评测/部署」页的 A/B 仍用 `pick="first"`（保持原有行为）。

同时把评测的合成预算收了一档（`max_mel_tokens` 600 ≈ 24 秒）：
短样本本来用不到这么多，它只在模型陷入重复循环时充当刹车，压低最坏情况的代价。

**音频处理链：做了什么、以及「为什么听起来发闷」**

先说一个**容易误解的事实**：IndexTTS2 的输出固定 **22050 Hz**，Nyquist 上限 11 kHz，
**11 kHz 以上不可能有能量** —— 这是模型设计。实测证据（都用真实数据量过）：

| 观察 | 结果 |
|---|---|
| 增强链是否损失高频？ | **不损失**。处理前后各频段能量完全一致（96.94 / 2.29 / 0.78 一模一样），唯一变化是重采样到模型要求的 22050 Hz |
| CFM LoRA 会不会让声音变闷？ | **不会，反而更亮**：8~11 kHz 能量 5.27% → 5.91%，谱质心 3304 → 3562 Hz |
| 官方示例音频呢？ | 8 kHz 以上几乎没有能量（0.05%）—— 说明这本来就是模型的工作区间 |

所以拿 48 kHz 的素材去比就会觉得「不通透」，但**不是处理链把高频裁掉了**。
能做的有两件事，都已落地：
...
| 处理 | 作用 |
|---|---|
| 高通 60 Hz | 去掉隆隆声/直流。它们不参与听感，却占动态余量、并把 RMS 响度算高（归一化因此压低语音本体） |
| **分频段降噪** | noisereduce 是宽带门限，会连齿音、气声一起压 —— 这是「闷」的常见来源。现在 **8 kHz 以上保留原始信号**，只压它以下的嘶声/嗡声/底噪 |
| TPDF 抖动 | 写 16-bit 前加抖动，打散与信号相关的量化失真（安静段、尾音处的「砂砾感」）。固定种子，保证同输入同字节 |

**② 后置（合成输出）——听感提亮**

合成页新增「输出后处理」区，默认开启一档温和设置，结果另存为 `*_polish.wav`
（原始输出保留，方便 A/B）：

| 处理 | 作用 |
|---|---|
| presence 提升 | 4.5 kHz 以上平滑抬升（默认 +2.5 dB）。这段决定「清楚」和「亮」，人耳最敏感 |
| 谐波激励 | 从 5 kHz 以上生成谐波、**只叠加 9 kHz 以上**的部分，缓解「高频被切断」的封闭感（默认 0.08） |
| 峰值对齐 | 对齐到 -1 dBFS，避免削波、也避免下游播放器自行衰减 |

实测同一段合成音频：

| 配置 | 4-8 kHz | 8-11 kHz | 谱质心 |
|---|---|---|---|
| 模型原始输出 | 7.22% | 5.91% | 3562 Hz |
| 后处理（默认档） | **11.51%** | **10.22%** | **3946 Hz** |
| 后处理（presence 4 + 激励 0.15） | 14.74% | 13.81% | 4173 Hz |
| 后处理（presence 0，只高通+峰值） | 7.24% | 5.93% | 3595 Hz（**音色不变**） |

合成页会显示处理前后的谱质心 —— **「亮了没有」有客观数字**。
**想要最大保真**：把「启用输出后处理」关掉，输出就与模型原始结果完全一致。

> 这里做的是**听感补偿**，不伪造外来细节：presence 是 EQ，激励是把**这段音频自身**
> 的高频做非线性延展。真实的 11 kHz 以上信息，模型没有生成，任何后处理也变不出来。
...
**whisper 的显存要按 fp32 算（实测）：**

| 档位 | 参数 | 实测预留 |
|---|---|---|
| base | 72 M | 0.42 GB |
| small | 241 M | 1.46 GB |
| medium（默认） | 762 M | **4.28 GB** |
| large-v3 | 1550 M | ~7.7 GB（8 GB 卡基本放不下） |

`openai-whisper` 的 `load_model()` 没有 dtype 参数，**一律 fp32 加载**；
想转 fp16（`.half()`）在这个版本上会因 LayerNorm 强制转 float 而直接报错，
所以省不下来。这也意味着 **medium 与推理引擎在 8 GB 卡上不可能共存** ——
这就是上面「识别前必须卸引擎」的原因；显存仍紧就往 small 退一档。
...
**参数记忆与配置档**（右栏「💾 参数记忆与配置档」面板）：

- **改动即记住**：本页任何参数改动（采样 / 情感 / 分句 / LoRA 选择与强度 / 输出后处理 / 音色库选择）都会自动落盘到 `outputs/state/`，**重启后自动恢复**——包括上传的参考音频（临时上传会被复制保存，重启不丢）。合成文本不记忆（内容不是配置）。
- **配置档**：把当前参数存成命名档位，存在 `outputs/presets/<名称>/`，**与「💾 预设」页及官方 `webui.py` 完全互通**；应用 / 删除都在面板内完成，删除需两步确认。
- **可控**：不想记忆就取消「记住参数改动」（跨重启保持关闭状态）；「清除记忆并恢复默认」一键回到出厂参数。
- 坏掉的记忆文件（手改 json / 磁盘损坏）会静默降级为无记忆，界面照常启动。
...
流程：**上传 → 体检打分 → 智能切片（选中即试听）→ 增强处理 → 入库 → 合成页直接调用**

- **体检打分**：每个指标对应推理链路里的一个真实约束（并非泛泛的「音质好不好」），例如官方推理只取前 15 秒，所以开头静音 / 噪声会直接吃掉有效信息；
- **智能切片**：从长音频里自动切出质量最佳的片段，**候选逐个列出；在下拉框里选中一个，预览播放器立即加载该片段**（扫描完成会自动试听评分最高的 #0）。试听与导出都**不会**替换主素材、不会作废候选列表 —— 想把某个片段当成新的工作对象，点「设为主素材」显式替换（顺手体检，可直接接着增强 / 入库）；
- **降噪归一**：一键增强处理；
- **音色库**：处理好的参考音频入库管理，合成页 / 批量页 / JSONL 任务均可引用。

**所有音频栏（输入与输出）都带下载按钮**：合成结果、片段预览、增强前后对比、批量试听、音色库试听等，播放器右下角即可下载另存。

**工作台的几个已知坑（都已处理）**

- **导出片段之后再点一次会失灵？** 已经修了（两轮）。第一轮修的是越界：导出曾把
  主素材换成切出的短片段，而候选偏移是**相对原始长音频**的秒数，再切就会越界
  产出 44 字节的空 wav —— 现在切分范围越界直接报错、换主素材自动作废候选、
  `save_audio` 拒绝写 0 采样点、体检不过不许入库。第二轮按用户反馈重构了交互：
  **「选中候选 → 直接试听」成为主路径**，试听与「导出该片段」只写预览播放器、
  不再替换主素材，候选列表保持有效可继续逐个试听；替换只发生在显式点
  「设为主素材」时（并顺手体检该片段）。
- **降噪是可选依赖**：没装 `noisereduce` 时 `_denoise` 原样返回（静默跳过），
  界面与数据集页都会明确提示，不会假装降噪做过了。
- **参考音频开头最要紧**：官方推理取**前 15 秒**而不是择优，所以切片时把最好的
  片段放在开头，或者直接用工作台的「设为主素材」换成一个干净的 8~15 秒片段。
...
> 合成页右栏的「💾 参数记忆与配置档」面板也能存 / 应用 / 删除预设（称为**配置档**），
> 两边读写的是同一个 `outputs/presets/` 目录，完全互通；区别是配置档面板
> 直接取合成页**实时值**，不用先跑一次生成。日常的「记住上次参数」由参数记忆
> 自动完成（见 [§7.2](#72-语音合成)），需要留档对比时才用预设 / 配置档。
...
用了一段时间之后，磁盘上会堆满各种产物：一键三连的评选试听与报告、A/B 评测输出、训练运行（adapter + 保险库 checkpoint + 日志）、LoRA 导出、数据集、音色库条目、工作台切片、单次合成与批量任务的音频、缓存。「🧹 清理」页把它们**逐条盘点**：每项列出名称、路径、体积、文件数、时间与来源（一键三连 / 手动），可单项删除、也可整类清空。

- **删除必须过确认弹窗**：弹窗里把每一项的路径与体积摆出来，明示「永久删除、不进回收站」才会执行；
- **底模与项目文件永不被波及**：`checkpoints/`、项目代码、`outputs/presets/`（预设）、`outputs/logs/`（日志）、`training_runs/base_manifest.json`（底座快照凭证）不出现在盘点里，删除路径做白名单 + realpath 包含性双重校验，伪造的越权 key 一律拒绝；
- **训练中的 run 自动锁定**：`status=running` 的训练记录按钮置灰，删除请求也会被服务端拒绝；
- **走正规删除通道**：数据集、训练记录、音色分别调用各自的删除 API，索引文件（如音色库 `index.json`）同步更新，不会留下悬空条目；
- 与「🖥 系统 → 磁盘清理」的分工：系统页是**快速清空整目录**（合成产物 / 工作台中间件 / 批量任务），清理页是**看清楚每一项再决定删不删**。
...
所有功能都有**可复现的回归测试**，不是「跑通了就完事」。每个探针自报通过 / 失败项数，失败时以非 0 退出码结束；下表数字为最近一次全量实跑结果（2026-09-20，RTX 4060 Laptop 8 GB），合计 **1428 项断言全部通过**。
...
| `tools/merge_probe.py` | 合并 / 挂载（双向数学对账，替身用真实 ModuleDict） | **29 / 29** |
| `tools/stage2_ui_probe.py` | UI + 后台执行器集成 | **20 / 20** |
| `tools/build_check.py` | 13 页构建校验 | ✅ |
| `tools/cleanup_probe.py` | 产物清理：11 类扫描发现 / 专用与通用删除通道 / 重复删除 skip / **越权与伪造 key 拒绝** / running run 锁定 | **18 / 18** |
| `tools/synth_state_probe.py` | 参数记忆：sanitize 钳制 / 往返一致 / **临时音频副本与稳定路径引用** / 失效音频丢弃 / 损坏 json 降级 / 配置档官方格式映射与 roundtrip | **30 / 30** |
| `tools/ui_output_check.py` | 事件回调返回值 vs 输出组件（源码级静态检查） | ✅ |
| `tools/lab_probe.py` | 参考音频工作台：候选片段范围 / 越界切分必须报错 / 空音频不许落盘与入库 / 高通 / 分频段降噪 / 抖动 / 输出提亮 | **57 / 57** |
| `tools/logging_probe.py` | 调试日志：落盘 / error.log 分离 / 全局兜底 / ui_guard / 静默时序 / LoRA 决策 / 面板取数 / 手册登记 | **80 / 80** |
| `tools/oneclick_probe.py` | 一键三连：选项自检 / 预设梯度 / 输入收集 / 切片并发 / 筛选闸门 / 调参构造 / 引擎互斥 / 报告 / 计划面板 / 参数登记 / **CPU 并行等价性** / 模型命名 / 阶段结果落盘 / 切片进度与可中断 / **模型加载前卸引擎** / **只解码一次** / **不转写不可训练样本** / **退化文本过滤** / **择优选短样本** | **256 / 256** |
| `tools/stage2_acceptance.py` | **真机完整闭环**（训练体系，见下） | **33 / 33** |
| `tools/oneclick_acceptance.py` | **一键三连真机端到端**（见下） | **70 / 70** |
| `tools/mem_acceptance.py` | **参数记忆真机端到端**（见下：自起服务器 + 无头浏览器 + 杀进程重启三轮） | **26 / 26** |
| `tools/lab_ui_acceptance.py` | **工作台切片交互真机端到端**（见下：选中即试听 / 试听导出不替换主素材 / 显式设为主素材 / 下载按钮） | **18 / 18** |
...
`tools/mem_acceptance.py` 自己起服务器、自己开无头浏览器，**杀进程再重启共三轮**，
26 项断言全过：

```
A 首次启动：读默认值（temperature=0.8 / 向量全 0）
  → 切情感模式到 8 维向量 → 点「快速档」+「随机一组」→ 界面与磁盘都变
  → 存配置档（官方 preset 格式，音频复制入档）
B 杀进程重启：temperature=0.7 / beams=1 / 向量逐位恢复 · chip「已恢复 34 项」
  → 下拉选中配置档 → 两步删除（第一次布防第二次真删）
  → 「清除记忆并恢复默认」→ 界面回默认且不再被重置回声覆写
  → 取消「记住参数改动」
C 再杀进程重启：记忆保持关闭（不恢复参数 · 开关仍为未勾选）
```

### 真机端到端验收 · 工作台切片交互

`tools/lab_ui_acceptance.py` 自造 40 秒合成语音、自起服务器 + 无头浏览器，
18 项断言全过：

```
路径载入长音频 → 主素材播放器带下载按钮
  → 扫描出 5 个候选 → 自动选中 #0 且预览立即加载（选中即试听）→ 预览带下载按钮
  → 下拉切到 #1 → 预览切换为 _seg1；主素材 href 全程保持长音频未被替换；候选列表仍在
  → 「导出该片段」→ 预览仍为 _seg1、主素材仍未替换
  → 「设为主素材」→ 此时主素材才替换为片段文件，候选列表按规则作废清空
```
...
**Q15：「🧹 清理」页删掉的文件还能找回来吗？**
不能。清理页的删除是**永久删除**（不进回收站），所以每次删除都会先弹出
确认框，把每一项的路径与体积列出来。清不掉的也放心：底模（`checkpoints/`）、
项目代码、预设、日志、底座快照凭证**永远不在清理范围**，训练进行中的 run
自动锁定；删除请求在服务端还会再做一次白名单与路径校验，伪造的越权请求
一律拒绝（有 18 项探针钉着，见 [§9](#9-测试与验证)）。
...
**Q17：识别（转写）特别慢，第一条就卡几分钟？**

按可能性从高到低：

1. **送去转写的音频本身就极长。** 一键三连现在只转写「能成为训练数据」的样本
   （1~20 秒），长音频原件会被跳过并在报告里说明。如果日志里出现
   `识别阶段跳过不可训练样本 N 条（最长 XXXX 秒）`，说明这条修复生效了。
2. **显存不够导致的静默降速。** 看识别开始前的这几行：
   ```
   引擎状态：已加载 → 已卸载=True · 空闲显存 6.93 GB
   加载 whisper-medium（device=cuda，空闲显存 6.93 GB）
   whisper-medium 就绪 · 加载 7.0s · 之后空闲 2.65 GB
   ```
   medium 实测要 4.28 GB；如果「空闲显存」在加载前就只有一两 GB，那说明还有
   别的东西占着显存（浏览器、IDE、残留的 python 进程），或者引擎没被卸掉。
   退一档到 `small`（1.46 GB）能明显缓解。
先看日志里识别开始前那几行：
...
**Q18：挂载 LoRA 时报错 `'ModuleDict' object has no attribute 'get'`？**
已修。`s2mel.models` 是 `torch.nn.ModuleDict`，**不是 dict 的子类、没有 `.get()`**，
所以挂载 **CFM** adapter 且强度非 1.0 时会在设强度那一步炸掉（GPT 走 `getattr` 所以一直没暴露）。
现在统一走 `guard.lora_target_module()`，对 dict 与 ModuleDict 都安全。

> 这个 bug 能藏住的原因值得记一笔：`merge_probe` 里用的替身是**普通 dict**，
> 而 `.get()` 在那边完全合法 —— 替身比真实对象宽松，测试反而在掩护 bug。
> 现在探针里的替身已换成真实的 `nn.ModuleDict`。

**Q19：合成的声音有点闷/不够亮，是高频被裁掉了吗？**
不是。模型的输出固定 22050 Hz（11 kHz 上限），这是设计；增强链实测频谱完全透明，
LoRA 反而让高频略多。见 [§7.2 音频处理链](#72-语音合成) 的实测表。
可用的手段是**听感提亮**（合成页「输出后处理」：presence + 空气感激励），
以及前置的**分频段降噪**（8 kHz 以上保留原始细节，整带降噪会让人声发闷）。

**Q20：LoRA 面板里看不到我刚训练的模型？**
...
├── theme.py          #   主题与通用组件
├── services/         #   引擎生命周期 / 推理 / 音频工作台 / 音色库 / 纠音 / 监控 / 产物盘点清理
├── tabs/             #   13 个功能页（每页一个文件）
└── training/         #   训练体系
