...
- **全部功能有可复现的回归测试**：**14 套**回归与静态校验、**1079 项断言全部通过**，另有两份真机端到端验收（训练体系 33 项 · 一键三连 61 项），均见 [§9 测试与验证](#9-测试与验证)。
...
所有功能都有**可复现的回归测试**，不是「跑通了就完事」。每个探针自报通过 / 失败项数，失败时以非 0 退出码结束；下表数字为最近一次全量实跑结果（2026-09-14，RTX 4060 Laptop 8 GB），合计 **1079 项断言全部通过**。
...
| `tools/oneclick_probe.py` | 一键三连：选项自检 / 预设梯度 / 输入收集 / 切片并发 / 筛选闸门 / 调参构造 / 引擎互斥 / 报告 / 计划面板 / 参数登记 | **135 / 135** |
| `tools/stage2_acceptance.py` | **真机完整闭环**（训练体系，见下） | **33 / 33** |
| `tools/oneclick_acceptance.py` | **一键三连真机端到端**（见下） | **61 / 61** |
...
流水线本身耗时 **3.8 分钟**（脚本含语料合成共 6.9 分钟），61 项断言全过：

```
引擎合成 40 句语料 → 拼成 1 条长音频（约 2.5 分钟）+ 10 条独立短音频
  → 一条长音频自动切成 15 片 · 共 26 条样本
  → 降噪归一（平均体检分 94）→ 逐片 whisper 转写 26 条（无输出 0 · 失败 0）
  → 筛选留用 25 条（约 3.08 分钟）→ 划分 train 21 / val 4
  → 离线特征提取 25 条
  → 自动调参：gpt→conservative(rank 4, 12 步) · cfm→conservative(rank 4, 6 步)
  → GPT/CFM 真训练，峰值显存 1.95 / 1.67 GB，各产出 3 个档位
  → 6 个候选真机打分排序 → 最优 reward 0.9604（WER 0.0333 · 声纹相似 0.9511）
...
>
> 顺带踩到一个真实陷阱并已修复：调参阶段若在引擎仍加载时读取空闲显存，
> 会把本来跑得动的配置全判为「显存不足」（首次验收就是这么停住的）。
> 现在特征提取一结束就卸载引擎，调参看到的是**训练时**的真实空闲显存。
