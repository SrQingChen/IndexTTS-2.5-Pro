...
- **全部功能有可复现的回归测试**：**15 套**回归与静态校验、**1237 项断言全部通过**，另有两份真机端到端验收（训练体系 33 项 · 一键三连 70 项），均见 [§9 测试与验证](#9-测试与验证)。
...
所有功能都有**可复现的回归测试**，不是「跑通了就完事」。每个探针自报通过 / 失败项数，失败时以非 0 退出码结束；下表数字为最近一次全量实跑结果（2026-09-15，RTX 4060 Laptop 8 GB），合计 **1237 项断言全部通过**。
...
| `tools/oneclick_probe.py` | 一键三连：选项自检 / 预设梯度 / 输入收集 / 切片并发 / 筛选闸门 / 调参构造 / 引擎互斥 / 报告 / 计划面板 / 参数登记 / **CPU 并行等价性** / 模型命名 / 阶段结果落盘 / **切片进度与可中断** | **204 / 204** |
